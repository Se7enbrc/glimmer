...
fingerprints…**. That action does **not** wipe the pin — it stages a comparison.
...
`Probing…`; if the host is unreachable it shows `<could not reach host>` — both
states keep the accept button disabled so the user can't trust an empty
