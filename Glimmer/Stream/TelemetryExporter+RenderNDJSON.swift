...
//  histogram-quantile estimator live in TelemetryExporter+Render.swift; the
//  process-level CPU/thread sampler it used to carry is ProcessMetrics.swift).
//  Split out to keep each unit focused and under the file-length budget; see
//  TelemetryExporter.swift for the exporter, gate, counters, and the snapshot
//  type.
//
