...
final class FramePacer: @unchecked Sendable {
    ...
    func stop() {
        ...
        // Fresh source-cadence window: a restart is a new source era, so an
        // inherited (half-filled) window must never judge the next link's
        // frames (FramePacer+SourceCadence.swift). Telemetry only.
